...
def pix_get_image(pix_view_urls, pix_meta_data):
    ...
    for pix_href in pix_view_urls:
        pix_saveas =  os.path.join(
            ...
            pixiv_savename(pix_href, **pix_meta_data)
        )
...
def get_view(pix_view_url, pix_save_to, **pix_view_meta):
    ...
    pix_browser.get(
        pix_view_url, 
        wait=gen_randwait(console_args.wait_time)
    )
    pix_presenter = lxml_spider(
        pix_browser.read(), 
    ...
    try:
        pix_safe_uid = pixiv_userid(
            spider_checkattr(pix_user_id, pix_presenter, href=xpath_pview_userid)
        )
    except:
