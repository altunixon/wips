...
class PillowImageEditor():
    def __init__(self, **kargs):
        ...
        self.compression_quality = kargs['quality'] if 'quality' in kargs.keys() \
                                   and str(kargs['quality']).isdigit() \
                                   else 90
        self.compression_optimize = kargs['optimize'] if 'optimize' in kargs.keys() \
    ...
    def Compress(self, img_path, **kargs):
        with Image.open(img_path) as iopend:
            try:
                ...
                else: iopend.save('%s_lq.%s' % tuple(img_path.rsplit('.',1)), 
                                  quality=self.compression_quality,
                                  optimize=self.compression_optimize)
            except:
    ...
    def Convert(self, img_path, **kargs):
        iformat = kargs['format'] if 'format' in kargs.keys() and \
                                    any(kargs['format'] == f for \
                                        f in self.accept_types.keys()) \
                                  else 'JPEG'
        imode   = kargs['mode'] if 'mode' in kargs.keys() and \
                                    any(kargs['mode'] == m for \
                                        m in self.conversion_mode.keys()) \
                                else 'RGB'
        ipath_noext = img_path.rsplit('.',1)[0]
        if iformat not in self.accept_types.keys():
            print_log('err', 'IMG_CONVERT - PATH="%s" to Un-Supported "%s" file type', img_path, iformat)
            raise Exception('Un-Supported "%s" file type, Try one of the listed below:\n%s' % (iformat, self.accept_types.keys()))
        else:
            with Image.open(img_path) as iopend:
                if iopend.format == iformat:
                    ...
                    try:
                        ...
                        if path.isfile(img_after):
                            img_after = ipath_noext + u'_coverted_to' + self.accept_types[iformat]
                        else:
                            pass
                        if path.isfile(img_after):
            ...
            if auto_scale >= 1:
                ...
                try:
                    ...
                    if self.delete_original: 
                        ...
                        iopend.save('%s_resized.%s' % tuple(img_path.rsplit('.',1)), 
                                    quality=self.compression_quality)
    ...
    def Trim(self, img_path, **kargs):
        ...
        fuzz = -(int(kargs['fuzz'])/100*255) if 'fuzz' in kargs.keys() \
                                             and str(kargs['fuzz']).isdigit() else -100 # ~45%
        scale= float(int(kargs['scale'])) if 'scale' in kargs.keys() \
                       and str(kargs['scale']).isdigit() else 2.0
        with Image.open(img_path) as iopend:
            if 'border' in kargs.keys() and str(kargs['border']) in color_pallete_rgb.keys():
                if iopend.mode == 'L': 
                ...
                if 'border' in kargs.keys() and isinstance(kargs['border'], tuple):
