...
from classes.utils              import get_siteconfigs, gen_randwait, gen_viewid, \
                                    gen_tagurl, merge_two_dicts, isindex, gen_savename
...
def do_post(view_href, table_name, save_path, view_refer, dl_verbose=True):
    global console_args
    global mech_database
    global post_page
    global external_source
    view_id = gen_viewid(view_href)
    view_url = urlPrefixer(view_href, view_refer)
    dlpost_returns = {
        'url': view_url, 
        'ok': False, 
        'saveas': None, 
    ...
    if post_images is not None or n_max > 0:
        ...
        for post_img in post_images:
            countdown(randint(1, console_args.wait), txt='Delay Start for "%s":' % post_img.src)
            ...
            if post_dlstat['ok']:
                ...
                dlpost_returns['saveas'] = image_saveas
            else:
    ...
    countdown(console_args.wait, txt = 'Post: "%s" [%s/%s] CoolDown' % (view_url, dlpost_returns['success'], n_max))
    return dlpost_result(**dlpost_returns)

import asyncio
import concurrent.futures
async def multi_post(posts_href, **post_data):
    conc_wrkr = int(post_data.pop('workers', console_args.workers))
    conc_verbose = False if conc_wrkr > 1 else console_args.verbose
    with concurrent.futures.ThreadPoolExecutor(max_workers=conc_wrkr) \
    as conc_executor:
        ...
        conc_futures = [
            ...
            for phref in posts_href
        ]
    return await asyncio.gather(*conc_futures)
...
def main(page_url, path_workdir, page_config, **options):
    ...
    page_tagname    = gen_tagurl(page_url, fifo=True)
        ...
        if not lazy_finish:
            while current_page is not None:
                ...
                if index_precheck.skip:
                    ...
                    current_page = urlPrefixer(
                            current_page_obj.next, page_url
                        ) \
                    ...
                    download_actual = []
                    for href_check in current_page_obj.views:
                        view_id = gen_viewid(href_check)
                        ...
                        if view_precheck.skip:
                            ...
                            download_actual.append(href_check)
                    conc_loop = asyncio.get_event_loop()
                    downloads_promised = conc_loop.run_until_complete(
                        multi_post(
                            download_actual, 
                            table = page_tagname, 
                            saveto = page_saveto, 
                    ...
                    for result_promised in downloads_promised:
                        if result_promised.ok:
                            download_returns['success'] += 1
                            if mech_database is not None:
                                mech_database.run(
                                    mode='insert row', 
                                    table=page_tagname, 
                                    view=result_promised.id,
                                    save=result_promised.saveas
                                )
                            else:
                                pass
                        else:
                            my_printer('err', 'POST  [CONC] - Tag: "%s" Refer: "%s"\nSave : "%s"\nPromised:\n%s', 
                                page_tagname, page_url, page_saveto, 
                                json.dumps(result_promised._asdict(), indent=4, sort_keys=True)
                            )
                            pass
                    my_printer('info', 
...
if __name__ == "__main__":
    ...
    args_parser.add_argument(
        '--workers', dest='workers', nargs='?', type=int, default=1, 
        help='Async Worker count, default=1.')
    args_parser.add_argument(
