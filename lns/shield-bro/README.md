...
- Main
    ...
    for i in $(curl https://yoraikun.wordpress.com/tny-chapters/ | sed -n '/div class="entry-content"/,/id="jp-post-flair"/p' | tr '<' '\n' | grep '^a '); do
        ...
        c_name=$(echo "$i" | awk -F '>' '{print tolower($NF)}' | sed 's/\:/ /g, s/&#8211; //g, s/  / /g, s/ /-/g')
        case $c_link in
        *pastebin*)
            c_id=$(echo "${c_link%/}" | awk -F '/' '{print $NF}')
            curl "http://pastebin.com/raw/$c_id" | sed 's/\n/<\/br>\n/g' > "./${c_name}_${c_id}.md"
        ;;
        *tsuki*)
            curl "$c_link" | sed -n '//,//p' > "./${c_name}-baka-tsuki.htm"
        ;;
        *bakahou*|*ohanashimi*|*yoraikun*)
            curl "$c_link" | sed -n '/class="entry-content"/,/\.entry-content/p' > "./${c_name}.htm"
        ;;
        esac
    done
    ```
